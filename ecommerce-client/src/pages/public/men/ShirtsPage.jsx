import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import ProductCard from '../../../components/shop/ProductCard';
import Loader from '../../../components/common/Loader';

const ShirtsPage = () => {
  const [products, setProducts] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const shirtsProducts = [
      { 
        id: 1, 
        name: 'Poplin Structural Panel Shirt', 
        brand: 'THEKOUR ATELIER',
        price: 79.99, 
        originalPrice: 110.00, 
        image: 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: 'Best Seller',
        color: 'White',
        rating: 4.8
      },
      { 
        id: 2, 
        name: 'Asymmetric Raw Coconut Button Blend', 
        brand: 'THEKOUR ATELIER',
        price: 89.99, 
        originalPrice: 125.00, 
        image: 'https://images.unsplash.com/photo-1576871337632-b9aef4c17ab9?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: 'New',
        color: 'Blue',
        rating: 4.7
      },
      { 
        id: 3, 
        name: 'Geometric Oversized Box Weight Shirt', 
        brand: 'THEKOUR ATELIER',
        price: 94.99, 
        originalPrice: 135.00, 
        image: 'https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: '',
        color: 'Black',
        rating: 4.6
      },
      { 
        id: 4, 
        name: 'Responsible Linen Relaxed Silhouette', 
        brand: 'THEKOUR ATELIER',
        price: 84.99, 
        originalPrice: 115.00, 
        image: 'https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: 'Sustainable',
        color: 'Beige',
        rating: 4.9
      },
      { 
        id: 5, 
        name: 'Oxford Classic Fit Shirt', 
        brand: 'THEKOUR ATELIER',
        price: 69.99, 
        originalPrice: 99.99, 
        image: 'https://images.unsplash.com/photo-1586363104862-3a5e2ab60d99?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: '',
        color: 'White',
        rating: 4.5
      },
      { 
        id: 6, 
        name: 'Denim Button Down', 
        brand: 'THEKOUR ATELIER',
        price: 99.99, 
        originalPrice: 149.99, 
        image: 'https://images.unsplash.com/photo-1576871337632-b9aef4c17ab9?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: 'Trending',
        color: 'Blue',
        rating: 4.8
      },
      { 
        id: 7, 
        name: 'Silk Blend Evening Shirt', 
        brand: 'THEKOUR ATELIER',
        price: 129.99, 
        originalPrice: 189.99, 
        image: 'https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: 'Luxury',
        color: 'Black',
        rating: 4.9
      },
      { 
        id: 8, 
        name: 'Cotton Linen Summer Shirt', 
        brand: 'THEKOUR ATELIER',
        price: 59.99, 
        originalPrice: 89.99, 
        image: 'https://images.unsplash.com/photo-1503342217505-b0a15ec3261c?w=400&h=500&fit=crop',
        category: 'Shirts',
        badge: '',
        color: 'White',
        rating: 4.6
      },
    ];
    setTimeout(() => { 
      setProducts(shirtsProducts); 
      setIsLoading(false); 
    }, 500);
  }, []);

  if (isLoading) return <Loader />;
  
  return (
    <div className="bg-gray-50 min-h-screen">
      {/* Hero Banner Section */}
      <div className="relative h-[50vh] min-h-[400px] bg-cover bg-center bg-no-repeat flex items-center justify-center"
           style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1586363104862-3a5e2ab60d99?w=1920&h=600&fit=crop)' }}>
        <div className="absolute inset-0 bg-gradient-to-r from-black/60 to-black/30"></div>
        <div className="relative text-center text-white px-4">
          <div className="inline-block px-4 py-1 bg-blue-500/20 backdrop-blur-sm rounded-full text-sm mb-4 border border-blue-400/30">
            👔 Premium Shirts
          </div>
          <h1 className="text-6xl md:text-7xl font-light mb-4 tracking-wide">Men's Shirts</h1>
          <p className="text-xl max-w-2xl mx-auto">Elevate your wardrobe with our premium shirt collection</p>
        </div>
      </div>

      {/* Breadcrumb */}
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="text-sm text-gray-500">
          <Link to="/" className="hover:text-black">Home</Link> / 
          <Link to="/men" className="hover:text-black ml-1">Men</Link> / 
          <span className="text-black ml-1">Shirts</span>
        </div>
      </div>

      {/* Shop by Style Section */}
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h2 className="text-2xl font-light text-center mb-8">Shop by Style</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {['Oxford', 'Linen', 'Denim', 'Silk'].map((style) => (
            <Link 
              key={style} 
              to={`/men/shirts/${style.toLowerCase()}`}
              className="group relative h-40 rounded-xl overflow-hidden"
            >
              <div className="absolute inset-0 bg-black/40 group-hover:bg-black/50 transition-colors z-10"></div>
              <img 
                src={`https://images.unsplash.com/photo-${style === 'Oxford' ? '1586363104862-3a5e2ab60d99' : style === 'Linen' ? '1594938298603-c8148c4dae35' : style === 'Denim' ? '1576871337632-b9aef4c17ab9' : '1591047139829-d91aecb6caea'}?w=400&h=200&fit=crop`}
                alt={style}
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
              />
              <div className="absolute inset-0 flex items-center justify-center z-20">
                <span className="text-white text-xl font-light uppercase tracking-wider">{style}</span>
              </div>
            </Link>
          ))}
        </div>
      </div>

      {/* Products Section */}
      <div className="max-w-7xl mx-auto px-4 pb-16">
        <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
          <div>
            <h2 className="text-3xl font-light">The Shirt Edit</h2>
            <p className="text-gray-500 text-sm mt-1">Essential styles for every occasion</p>
          </div>
          <div className="flex gap-3">
            <select className="px-4 py-2 border rounded-lg text-sm focus:outline-none focus:border-black">
              <option>Featured</option>
              <option>Price: Low to High</option>
              <option>Price: High to Low</option>
              <option>Best Rated</option>
              <option>New Arrivals</option>
            </select>
          </div>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {products.map(product => (
            <ProductCard key={product.id} product={product} viewMode="grid" />
          ))}
        </div>
        
        {/* Explore More Button */}
        <div className="text-center mt-12">
          <Link 
            to="/men" 
            className="inline-block px-8 py-3 border-2 border-black text-black rounded-full font-medium hover:bg-black hover:text-white transition-colors"
          >
            Shop All Shirts →
          </Link>
        </div>
      </div>
    </div>
  );
};

export default ShirtsPage;
