...
const LoginPage = () => {
    ...
    // Admin credentials
    const ADMIN_EMAIL = 'admin@thekour.com';
    ...
    const handleSubmit = async (e) => {
        ...
        setTimeout(() => {
            ...
            if (email && password.length >= 6) {
                if (savedUserEmail && email === savedUserEmail) {
                    // Existing user
                    localStorage.setItem('isAuthenticated', 'true');
                    localStorage.setItem('isAdmin', 'false');
                    localStorage.setItem('userRole', 'user');
                } else {
                    // Demo login for any email with password >=6 chars
                    localStorage.setItem('isAuthenticated', 'true');
                    localStorage.setItem('isAdmin', 'false');
                    localStorage.setItem('userEmail', email);
        ...
        <AuthLayout title="Welcome Back" subtitle="Sign in to your account">
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end">
                    <Link to="/forgot-password" className="text-sm text-gray-600 hover:text-black">Forgot password?</Link>
                </div>
                <button 
                    type="submit" 
                    disabled={loading} 
                ...
                <p className="text-center text-sm">
                    Don't have an account? <Link to="/register" className="font-medium underline">Create Account</Link>
                </p>
            </form>
        </AuthLayout>
    );
